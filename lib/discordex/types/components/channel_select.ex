defmodule Discordex.Types.Components.ChannelSelect do
  @moduledoc """
  Discord Channel Select component.

  See: https://discord.com/developers/docs/interactions/message-components#channel-select
  """

  alias Discordex.Types.DefaultValue
  ...
  def new(custom_id, opts) when is_binary(custom_id) and is_list(opts) do
    ...
    with :ok <- validate_custom_id(custom_id),
         ...
         :ok <- validate_channel_types(opts[:channel_types]),
  ...
  def new(_, _), do: {:error, :invalid_channel_select}
  ...
  defp validate_channel_types(nil), do: :ok
  defp validate_channel_types(values) when is_list(values) and length(values) <= 25, do: :ok
  defp validate_channel_types(_), do: {:error, :invalid_channel_types}
...
defimpl Discordex.Types.Encodable, for: Discordex.Types.Components.ChannelSelect do
  def to_map(select) do
    ...
    |> maybe_put_channel_types(select.channel_types)
  ...
  defp maybe_put_default_values(map, values) do
    Map.put(map, :default_values, Enum.map(values, &Discordex.Types.Encodable.to_map/1))
  end

  defp maybe_put_channel_types(map, []), do: map
  defp maybe_put_channel_types(map, channel_types) do
    Map.put(map, :channel_types, channel_types)
  end
end
