defmodule Discordex.Discord.Components.Section do
  @moduledoc """
  ...
  alias Discordex.Discord.Components.{Button, TextDisplay, Thumbnail}
...
defimpl Discordex.Discord.Encodable, for: Discordex.Discord.Components.Section do
  def to_map(section) do
    %{
      type: 9,
      components: Enum.map(section.components, &Discordex.Discord.Encodable.to_map/1),
      accessory: Discordex.Discord.Encodable.to_map(section.accessory)
    }
    |> Discordex.Discord.Encodable.Helpers.maybe_put(:id, section.id)
  end
end
