defmodule Discordex.Discord.Interaction do
  @moduledoc """
  ...
  alias Discordex.Discord.{User, GuildMember, Channel, Guild, Message, Entitlement}
  ...
  defp decode_resolved_data(resolved) do
    struct(__MODULE__.ResolvedData, %{
      ...
      roles: decode_resolved_map(resolved["roles"], &Discordex.Discord.Role.decode/1),
      ...
      attachments: decode_resolved_map(resolved["attachments"], &Discordex.Discord.Attachment.decode/1)
    })
  end
...
defimpl Discordex.Discord.Encodable, for: Discordex.Discord.Interaction do
  alias Discordex.Discord.Interaction.Type
  ...
  defp maybe_put_context(map, context) do
    Map.put(map, :context, Discordex.Discord.Interaction.ContextType.encode(context))
  end
end
