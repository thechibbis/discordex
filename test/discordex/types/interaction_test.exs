defmodule Discordex.Types.InteractionTest do
  ...
  describe "decode/1" do
    ...
    test "decodes nested types (user, member, guild, channel, message)" do
      ...
      assert {:ok, interaction} = Interaction.decode(payload)
      assert %Discordex.Types.User{username: "alice"} = interaction.user
      assert %Discordex.Types.GuildMember{nick: "AliceNick"} = interaction.member
      assert %Discordex.Types.Guild{name: "TestGuild"} = interaction.guild
      assert %Discordex.Types.Channel{name: "general"} = interaction.channel
      assert %Discordex.Types.Message{content: "hello"} = interaction.message
    end
    ...
    test "decodes entitlements" do
      ...
      assert %Discordex.Types.Entitlement{sku_id: "sku1"} = Enum.at(interaction.entitlements, 0)
      assert %Discordex.Types.Entitlement{sku_id: "sku2"} = Enum.at(interaction.entitlements, 1)
    end
  ...
  describe "Encodable.to_map/1" do
    test "encodes interaction to Discord API map" do
      ...
      map = Discordex.Types.Encodable.to_map(interaction)
      assert map.id == "123"
    ...
    test "encodes nested resource types (user, guild, entitlements)" do
      interaction = %Interaction{
        ...
        app_permissions: "0",
        entitlements: [
          %Discordex.Types.Entitlement{id: "e1", sku_id: "sku1", application_id: "app1", type: 1}
        ],
        authorizing_integration_owners: %{},
        attachment_size_limit: 0,
        user: %Discordex.Types.User{id: "u1", username: "testuser"},
        guild: %Discordex.Types.Guild{id: "g1", name: "TestGuild"}
      }
    ...
    test "round-trip: decode then encode is lossy but preserves key fields" do
      ...
      encoded = Discordex.Types.Encodable.to_map(decoded)
